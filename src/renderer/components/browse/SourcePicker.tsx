...
import type { SuggestedSource, MarketplaceSourceConfig } from '@shared/types';
...
export function SourcePicker({ onSourcesAdded }: SourcePickerProps) {
  const [userSources, setUserSources] = useState<MarketplaceSourceConfig[]>([]);
  const [checked, setChecked] = useState<Set<string>>(new Set());
  const [adding, setAdding] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const suggested = SUGGESTED_SOURCES_MANIFEST.sources;
  ...
  return (
    <div>
      <h4 className="text-sm font-medium text-sand-text mb-2">Add plugin sources (marketplaces)</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-sand-muted mt-0.5 font-mono truncate">
                      {source.url}
                    </p>

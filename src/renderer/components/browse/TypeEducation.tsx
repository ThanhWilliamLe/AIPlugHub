...
const TYPE_ROWS: TypeRow[] = [
  ...
  },
  {
    ...
    whatToExpect:
      'A separate AI personality tuned for one task (reviewing code, debugging, etc). You invoke it by name. May use more tokens when active.',
  },
  {
    type: 'MCP Server',
    whatItDoes: 'Connects your AI to an external tool',
    whatToExpect:
      'Runs a background process your AI calls to reach databases, APIs, browsers, etc. Uses system resources (memory, CPU) while active. May require API keys or credentials.',
  },
  {
    type: 'Hook',
    whatItDoes: 'Automation that runs on AI events',
    whatToExpect:
      'Executes automatically before/after AI actions (like pre-commit checks). Runs without asking \u2014 can block your workflow if misconfigured.',
  },
  {
    type: 'Context File',
    whatItDoes: 'Background info the AI reads',
    whatToExpect:
      'A document the AI loads as context. Affects how it responds but isn\u2019t a "feature" you interact with. Increases context window usage.',
  },
];

...
describe('SuggestedSourcesManifest', () => {
  it('has version 2', () => {
    expect(SUGGESTED_SOURCES_MANIFEST.version).toBe(2);
  });
  ...
  it('featured plugin sourceIds reference a suggested source or the built-in official source', () => {
    const sourceIds = new Set(SUGGESTED_SOURCES_MANIFEST.sources.map((s) => s.sourceId));
    sourceIds.add('claude-plugins-official'); // Built-in source, always available
    for (const plugin of SUGGESTED_SOURCES_MANIFEST.featured) {
